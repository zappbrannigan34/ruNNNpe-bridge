...
3. Open the app.
4. On first launch, grant all requested permissions (BLE, Health Connect, notifications; optional location for virtual route anchor) and allow battery optimization exemption.
5. Tap `Find RUNN & Start`.

The app also runs automatic maintenance checks on updates and periodically while in use:

- Re-checks required BLE + Health Connect + notification permissions.
